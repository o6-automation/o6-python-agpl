...
def node_detail_lines(node, max_w):
    ...
    if isinstance(node, VariableNode):
        for pair in [
            ...
            tryread(
                "ArrayDimensions", lambda: node(attr=o6.AttributeId.ARRAYDIMENSIONS)
            ),
            tryread("AccessLevel", lambda: node(attr=o6.AttributeId.ACCESSLEVEL)),
            tryread(
                "UserAccessLevel", lambda: node(attr=o6.AttributeId.USERACCESSLEVEL)
            ),
            tryread(
                "MinSamplingInterval",
                lambda: node(attr=o6.AttributeId.MINIMUMSAMPLINGINTERVAL),
            ),
        ...
        for pair in [
            ...
            tryread(
                "DataTypeDefinition",
                lambda: node(attr=o6.AttributeId.DATATYPEDEFINITION),
            ),
        ]:
            if pair:
                pairs.append(pair)
        ...
        for pair in [
            tryread(
                "ContainsNoLoops", lambda: node(attr=o6.AttributeId.CONTAINSNOLOOPS)
            ),
...
def main(stdscr):
    ...
    with Client(endpoint_url) as client:
        ...
        history = []
        path_segments = [
            str(client.read(node._nodeid, attribute_id=o6.AttributeId.BROWSENAME))
        ]
        selected = 0
        message = ""
        filter_query = ""
                            ...
                            try:
                                ...
                                new_segments = [
                                    str(
                                        client.read(
                                            cur._nodeid,
                                            attribute_id=o6.AttributeId.BROWSENAME,
                                        )
                                    )
                                ]
                                for seg in segments:
