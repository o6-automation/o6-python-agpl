...
class clientDemo:
    ...
    def move_joints_smoothly(
        ...
        if "Error" in current_angles:
            print(
                "[o6 Python] [ERROR] Cannot move smoothly: Failed to read current Feedback angles."
            )
            return
    ...
    def live_monitor(self) -> None:
        ...
        watched = [
            ...
            (
                ...
                lambda v: (
                    "Red detected"
                    if str(v) == "1"
                    else "Blue detected" if str(v) == "4" else "None/Waiting"
                ),
            ),
        ...
        def render_line() -> None:
            ...
            print(
                f" -> Robot: {state['robot']:<8} | "
                f"Sensor: {state['sensor']:<14} | "
                f"Joints: {joints_str}"
            )
