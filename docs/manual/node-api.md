...
A running example is the fastest way to learn. The snippets below assume a server that exposes an object with a few variables and a method, similar to the [example server](../tutorials/setup.md) used by the tutorials:
...
For the underlying service calls (when you need full control over the request payload), see the [Client](client/index.md) and [Service API reference](../api_reference/client.md).
