Client Browsing : Adress Space Traversal
==========================
...
The example is wired against `basic_server.py` — start that script in
...
real OPC UA client: server-assigned NodeIds are often not known at
compile time, and the *browse name* of a node is the stable
...
hands back a `Node` handle, and `Node` itself supports dot syntax
(`.Plant`) and browse-path syntax (`["/Objects/Plant"]`) so you can
walk the tree without manually tracking NodeIds.

## 1. Connection Setup
Same as in `client_basic.py` and `lowlevel/client.py`: build the
...
## 2. Sync Style: Get a Node Directly
...
## 3. Sync Style: Dot Traversal
`node.Child` walks one level into the address space by browse name —
the same way you'd navigate Python objects. The lookup happens on
the client (no extra round-trip) the second time, because the
children are cached on the `Node` instance.
...
## 4. Sync Style: Enumerate Children with `client.browse(...)`
`client.browse(node, ...)` walks one level out from `node` and
returns a list of `ReferenceDescription`s. The `resultMask` keyword
...
## 5. Sync Style: Read and Write Through the `Node` Handle
Calling a `Node` with no arguments (`node()`) returns the current
value of its `Value` attribute; calling it with a value
(`node(value)`) writes that value. The SDK infers the OPC UA data
...
## 6. Async Style: The Same Walk, Inside an Event Loop
