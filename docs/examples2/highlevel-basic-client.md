...
same trick used in the other examples: it makes the URL match the
hostname the server advertises during discovery, which keeps
encrypted endpoints happy (unencrypted servers like `basic_server.py`
don't care, but the convention is worth keeping).
...
`client.write(nodeid, value)` assigns a new one. Both are 1:1 with
...
connection failures surface as `StatusCodeError`.
...
the same order; `client.write({...])` accepts a dict mapping each
...
plant_id = next(
    r.nodeId for r in refs if r.browseName.name == "Plant"
)
...
The flow is: create a subscription with a publishing interval
...
given NodeId — handy for introspection. The `Node` object can be
...
returning a list of `ReferenceDescription`s. The `resultMask`
keyword controls which fields each reference carries back
(browse name, display name, node class, type definition, ...) — the
default is `0`, which is fast but returns empty names; pass `63`
(or `BrowseResultMask(63)`) to get every field populated.
...
```python
    ...
    with Client(endpoint_url) as client:
        ...
        # `lowlevel/client.py` — use `resultMask=63` to get names.)
