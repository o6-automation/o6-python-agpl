...
# Short type codes for serialized node hierarchy.  "N" is the generic node;
# "D" marks DataType nodes (which may carry a built Python type at runtime).
...
def _serialize_ns_node(ns_node: Any) -> tuple[str, str, dict[str, Any]]:
    ...
    type_code = _NS_NODE_TYPE_CODES.get(type(ns_node).__name__, "N")
...
def _serialize_namespace_types(descriptor: Any, xml_sha256: str) -> str:
    ...
    for sd in descriptor._structure_descriptions:
        ...
        if str(sd.data_type_id) in descriptor._abstract_data_types:
            struct_dict["is_abstract"] = True
    ...
    for ed in descriptor._enum_descriptions:
        ...
        enum_dict: dict[str, Any] = {"fields": enum_fields}
        if str(ed.data_type_id) in descriptor._abstract_data_types:
            enum_dict["is_abstract"] = True
        enums.append((str(ed.data_type_id), str(ed.name), enum_dict))
...
def _update_nodesets_init() -> None:
    content = "# Copyright 2026 (c) o6 Automation GmbH\n# AUTO-GENERATED — DO NOT EDIT\n# Run tools/update_ns.py to regenerate.\n"
