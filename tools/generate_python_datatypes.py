...
def _parse_statuscode_csv(fh) -> list[tuple[str, int, str]]:
    """Parse a StatusCode CSV (name, hex value[, description])."""
    import csv as _csv
...
def _generate_statuscode_stub(fh) -> list[str]:
    """Return .pyi lines defining the StatusCode IntFlag class."""
    members = _parse_statuscode_csv(fh)
    lines = [
        '# --- StatusCode (autogenerated from StatusCode.csv) ---',
        '',
        'class StatusCode(enum.IntFlag, boundary=enum.KEEP):',
    ...
    for name, value, desc in members:
        comment = f'  # {desc}' if desc else ''
        lines.append(f'    {name} = 0x{value:08X}{comment}')
    lines.append('    # fmt: on')
    return lines
...
def main() -> None:
    ...
    parser.add_argument('--statuscode-csv', type=argparse.FileType('r'), default=None,
                        help='StatusCode.csv file (e.g. deps/open62541/tools/schema/StatusCode.csv). '
                             'If provided, the StatusCode IntFlag class is emitted into the stub.')
    ...
    # Header
    out.write('# Copyright 2026 (c) o6 Automation GmbH\n')
    out.write('# This file is auto-generated by tools/generate_python_datatypes.py\n')
    ...
    out.write('    DateTime, NodeId, NodeIdLike, ExpandedNodeId,\n')
    out.write('    QualifiedName, LocalizedText, DataValue,\n')
    out.write(')\n')
    out.write('\n')

    # Emit the StatusCode IntFlag class before all other enums/structs so the
    # rest of the generated stub can reference it.
    if args.statuscode_csv is not None:
        out.writelines(line + '\n' for line in _generate_statuscode_stub(args.statuscode_csv))
        out.write('\n')
