...
class NamespaceDataTypeNode(NamespaceNode):
    def __init__(self, nodeid: Any, datatype: type | None = None) -> None:
    ...
    def __call__(self) -> Any:
        ...
        raise TypeError(
            f"{self._nodeid!r} is an abstract or unresolved DataType and cannot be "
            "instantiated. Use o6.NodeId(<wrapper>) to obtain its NodeId."
        )


def _clone_container(src: SimpleNamespace) -> SimpleNamespace:
    """Recursively duplicate a node-hierarchy container.

    Produces a new ``SimpleNamespace`` tree where every ``NamespaceNode`` /
    ``NamespaceDataTypeNode`` wrapper is a fresh instance carrying the same
    ``_nodeid`` (and ``_datatype=None`` for datatype nodes — to be re-bound
    by ``_link_datatypes`` per owner).  Non-node attributes are copied by
    reference (they are immutable strings or already-shared metadata).
    """
    dst = SimpleNamespace()
    for name, child in vars(src).items():
        if isinstance(child, NamespaceDataTypeNode):
            new = NamespaceDataTypeNode(child._nodeid, None)
            for sub_name, sub_child in vars(child).items():
                if sub_name in ("_nodeid", "_datatype"):
                    continue
                if isinstance(sub_child, NamespaceNode):
                    setattr(new, sub_name, _clone_node(sub_child))
                else:
                    setattr(new, sub_name, sub_child)
            setattr(dst, name, new)
        elif isinstance(child, NamespaceNode):
            setattr(dst, name, _clone_node(child))
        else:
            setattr(dst, name, child)
    return dst


def _clone_node(src: NamespaceNode) -> NamespaceNode:
    """Recursively duplicate a NamespaceNode (or NamespaceDataTypeNode)."""
    if isinstance(src, NamespaceDataTypeNode):
        new: NamespaceNode = NamespaceDataTypeNode(src._nodeid, None)
    else:
        new = NamespaceNode(src._nodeid)
    for name, child in vars(src).items():
        if name in ("_nodeid", "_datatype"):
            continue
        if isinstance(child, NamespaceNode):
            setattr(new, name, _clone_node(child))
        else:
...
class Namespace:
    def __init__(
        ...
        self._abstract_data_types: set[str] = set()
    ...
    def _copy_for_rebuild(self) -> Namespace:
        ...
        namespace indices.  Copies metadata and original NodeIds; node
        hierarchy containers are deep-cloned so per-owner remaps do NOT
        mutate the global descriptor's ``datatypes`` wrappers (which would
        cause last-``append()``-wins on ``node._datatype`` across multiple
        in-process clients/servers).  Built capsules and types are not
        carried over — they are rebuilt by ``_build_types``."""
        d = Namespace(
        ...
        d._abstract_data_types = set(self._abstract_data_types)
        # Deep-clone the node-hierarchy containers so per-owner mutations
        # (e.g. _link_datatypes assigning ``_datatype``) stay local to this
        # copy.  Node NodeIds (XML-index strings) are safe to share by value.
        d.objects = _clone_container(self.objects)
        d.datatypes = _clone_container(self.datatypes)
        d.eventtypes = _clone_container(self.eventtypes)
        d.ifacetypes = _clone_container(self.ifacetypes)
        d.objtypes = _clone_container(self.objtypes)
        d.reftypes = _clone_container(self.reftypes)
        d.vartypes = _clone_container(self.vartypes)
        d.views = _clone_container(self.views)
        return d
    ...
    def _add_structure_description(
        ...
        if isinstance(struct_data, dict) and struct_data.get("is_abstract"):
            self._abstract_data_types.add(str(sd.data_type_id))
    ...
    def _add_enum_description(
        ...
        if isinstance(enum_data, dict) and enum_data.get("is_abstract"):
            self._abstract_data_types.add(str(ed.data_type_id))
