...
def _parse_datatype_el(
    ...
    is_abstract = dt_el.get("IsAbstract", "false").lower() == "true"
    ...
    if enum_def is not None:
        _parse_enum_def(enum_def, nodeid, browse_name, descriptor, is_abstract)
        return
...
def _parse_struct_fields(
    ...
    for f in parent_el.findall(f".//{{{_XSD}}}{field_tag}"):
        ...
        if arr_dim:
            fd["array_dimensions"] = [int(x) for x in arr_dim.split() if x]
...
def _parse_structure_def(
    ...
    descriptor._add_structure_description(
        nodeid,
        browse_name,
        {"structure_type": struct_type, "fields": fields, "is_abstract": is_abstract},
...
def _parse_union_def(
    ...
    if fields:
        descriptor._add_structure_description(
            nodeid,
            browse_name,
            {"structure_type": 2, "fields": fields, "is_abstract": is_abstract},
            default_encoding_id=enc_map.get(nodeid),
        )
...
def _collect_enum_fields(
    ...
    for f in parent_el.findall(f".//{{{_XSD}}}{field_tag}"):
        ...
        dn = f.get("DisplayName") or name.upper()
        if dn:
            fd["display_name"] = dn
        desc = f.get("Description", "")
        if desc:
...
def _parse_enum_def(
    ...
    if fields:
        descriptor._add_enum_description(
            nodeid,
            browse_name,
            {"fields": fields, "is_abstract": is_abstract},
        )
...
def _parse_inline_def(
    ...
    if "Value" in first.attrib and "DataType" not in first.attrib:
        ...
        descriptor._add_structure_description(
            ...
            browse_name,
            {
                "structure_type": 1 if has_optional else 0,
...
def _parse_v103_def(
    ...
    if struct_fields_el:
        ...
        descriptor._add_structure_description(
            nodeid,
            browse_name,
            {"structure_type": 0, "fields": fields, "is_abstract": is_abstract},
        )
        return
...
def _make_ns_node(info: _NodeInfo) -> Any:
    """Instantiate the appropriate Namespace node wrapper for *info*."""
    nid = info.nodeid
    if info.kind == "datatype":
        return NamespaceDataTypeNode(nid, None)
    return NamespaceNode(nid)
