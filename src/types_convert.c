...
PY2UA_statuscode(PyObject *obj, UA_StatusCode *p) {
    /* StatusCode is a Python IntFlag (subclass of int), so any int-like
...
PY2UA_nodeid(PyObject *obj, UA_NodeId *p) {
    ...
    if(Py_TYPE(obj) == pyUAExpandedNodeId) {
        ...
        UA_ExpandedNodeId_init(&tmp);
        PyObject *res_obj = PY2UA_expandednodeid(obj, &tmp);
        if(!res_obj) {
            UA_ExpandedNodeId_clear(&tmp);
            return NULL;
        }
        if(!UA_ExpandedNodeId_isLocal(&tmp)) {
            UA_ExpandedNodeId_clear(&tmp);
            PyErr_SetString(PyExc_TypeError,
                            "ExpandedNodeId must be local to convert to NodeId");
            return NULL;
        }
        res = UA_NodeId_copy(&tmp.nodeId, p);
        UA_ExpandedNodeId_clear(&tmp);
        if(res != UA_STATUSCODE_GOOD) {
            PyErr_NoMemory();
            return NULL;
...
PY2UA_qualifiedname(PyObject *obj, UA_QualifiedName *p) {
    ...
    if(Py_TYPE(obj) == pyUAQualifiedName) {
        ...
        if(res == UA_STATUSCODE_GOOD && p->name.length == 0 && p->name.data) {
            UA_String_clear(&p->name);
        }
        return (res == UA_STATUSCODE_GOOD) ? Py_None : NULL;
    }
     ...
     * See the matching note in the STRING case of PY2UA. */
    if(p->name.length == 0 && p->name.data) {
        UA_String_clear(&p->name);
    }
    return Py_None;
}
...
UA2PY_variant(UA_Variant *v) {
    ...
    // When arrayDimensions[i] == 0 the OPC UA spec uses it as an "any-length"
    // placeholder — substitute the actual arrayLength so the numpy array is
    // populated correctly.  This typically happens when the server stores the
    // node's ValueRank dimension descriptor inside the subscription DataValue
    // (e.g. arrayDimensions=[0] for a 1-D variable-length array).
    int ndims;
    npy_intp dims[10];
    if(v->arrayDimensionsSize == 0) {
        ...
        size_t declared_total = 1;
        for(size_t i = 0; i < v->arrayDimensionsSize; i++) {
            dims[i] = (npy_intp)v->arrayDimensions[i];
            declared_total *= (v->arrayDimensions[i] ? v->arrayDimensions[i] : 1);
        }
        // If all declared dimensions are zero markers (or their product doesn't
        // match arrayLength) fall back to a flat 1-D view of the actual data.
        if(declared_total == 0 || declared_total != v->arrayLength) {
            ndims = 1;
            dims[0] = (npy_intp)v->arrayLength;
        }
    }
...
ua2py_inner(void *p, const UA_DataType *type) {
    ...
    case UA_DATATYPEKIND_STATUSCODE: {
        /* StatusCode is an IntFlag enum — call the enum class with the uint32. */
        UA_UInt32 val = *(UA_UInt32*)p;
        PyObject *intObj = PyLong_FromUnsignedLong((unsigned long)val);
        if(!intObj) return NULL;
        PyObject *result = PyObject_CallOneArg((PyObject*)pyUAStatusCode, intObj);
        Py_DECREF(intObj);
        return result;
    }
    ...
    for(size_t i = 0; i < type->membersSize; i++) {
        ...
        if(m->isArray) {
            ...
            if(uaValue) {
                ...
                uaValue = (char*)uaValue + sizeof(size_t);
...
py2ua_inner(PyObject *obj, void *p, const UA_DataType *type) {
    ...
    case UA_DATATYPEKIND_XMLELEMENT: {
        ...
        if(size < 0)
            return NULL;
        /* Empty Python strings are encoded on the wire as a NULL String
         * (length=-1), matching the OPC UA convention used by most clients
         * including open62541's C API. Encoding them as a 0-length String
         * (length=0, data=non-NULL) confuses some servers that treat the
         * empty IndexRange/DataEncoding/etc. as a real filter and then
         * silently fail to deliver any data. */
        if(size == 0) {
